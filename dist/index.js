...
var PiRpcProcess = class _PiRpcProcess {
  ...
  async getLastAssistantText() {
    ...
    return String(res.data?.text ?? res.data ?? "");
  }
  async getCommands() {
...
var PiAcpSession = class {
  ...
  async prompt(message, images = []) {
    ...
    const turnPromise = new Promise((resolve4, reject) => {
      const queued = { message: expandedMessage, originalMessage: message, images, resolve: resolve4, reject };
      if (this.pendingTurn) {
...
async function promptForTitle(proc, prompt) {
  let off = () => {
  };
  let timeout;
  const agentEnd = new Promise((resolve4, reject) => {
    timeout = setTimeout(() => reject(new Error("Timed out waiting for title generation")), 3e4);
    off = proc.onEvent((ev) => {
      if (String(ev.type ?? "") === "agent_end") {
        off();
        off = () => {
        };
        resolve4();
...
function buildTitlePrompt(userMessage, assistantText) {
  return [
...
function trimForTitlePrompt(text) {
  const normalized = text.replace(/\s+/g, " ").trim();
  return normalized.length > 6e3 ? `${normalized.slice(0, 6e3).trim()}\u2026` : normalized;
}
function sanitizeGeneratedTitle(raw) {
  let title = raw.split(/\r?\n/).map((line) => stripTitleControlChars(line).trim()).find((line) => line.length > 0) ?? "";
...
function stripTitleControlChars(text) {
  return Array.from(text, (ch) => {
    const code = ch.charCodeAt(0);
    return code >= 0 && code <= 8 || code === 11 || code === 12 || code >= 14 && code <= 31 || code === 127 ? " " : ch;
  }).join("");
}
function hasSessionName(state) {
